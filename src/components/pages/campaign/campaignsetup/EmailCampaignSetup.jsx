...
import { EmailCampaignSetupValidation } from '../../../../helper/ValidationHelper/Validation';
import Swal from 'sweetalert2';
import { IsAccessibleMethod } from '../../../../guard/Rbac';
import PermissionSets from '../../../../guard/Method';
const EmailCampaignSetup = ({ isSelectAll, setCurrentPage, setCheckBoxList, checkboxList, currentSelectedTab }) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: EmailCampaignSetupValidation,
    ...
    const handleSubmit = (data) => {
        ...
        EmailCampaignCreate.mutate(sendObj)
    }


    const EmailCampaignCreate = useMutation(
        (data) => {
            if (currentSelectedTab == "brideandgroom") {
                return CampaignServices.EmailCampaignBrideandgroom(data)
            }
            else if (currentSelectedTab == "organization" || currentSelectedTab == "franchise") {
                return CampaignServices.EmailCampaignTenant(data)
            }
        },
        {
            onSuccess: (data) => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        EmailCampaignSend.mutate({ id: data?.data?.id });
                    } else {
        ...
        <>
            ...
            <form onSubmit={formik.handleSubmit}>
                ...
                <FormGroup className='common-formgroup'>
                    ...
                    {isEmailTemplateLoad ? <ButtonLoader /> : <Select
                        className={
                        ...
                        isDisabled={isEmailTemplateLoad}
                        isLoading={isEmailTemplateLoad}
                ...
                <div className='campaign-body-wrap'>
                    ...
                    <FormGroup className='common-formgroup text-end'>
                        ...
                        >  <Button disabled={EmailCampaignCreate?.isLoading || EmailCampaignSend?.isLoading} type='submit' className='btn btn-style1'>{EmailCampaignCreate?.isLoading || EmailCampaignSend?.isLoading ? <ButtonLoader /> : "Create Campaign"}</Button></IsAccessibleMethod>
                    </FormGroup>
                </div>
            </form>
        </>
