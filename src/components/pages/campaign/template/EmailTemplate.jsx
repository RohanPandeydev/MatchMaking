...
const EmailTemplate = () => {
    ...
    const handleEditEmail = (e, id, data) => {
        setEditId(id)
        setEditDetails(data)
        setTitle(data?.title)
        setText(data?.body)
    ...
    const handleAddField = (e, field) => {
        ...
        if (/<p>.*<\/p>/.test(text)) {
            // If <p> tag exists, append the field in bold
            updatedText = text.replace(
                /<p>(.*?)<\/p>/,
                (match, content) => `<p>${content} <b>{{${field.value}}}</b></p>`
            );
        } else {
            // If <p> tag does not exist, add <p> tag with the field in bold
            updatedText = `<p><b>{{${field.value}}}</b></p>`;
        }
    ...
    const handleSubmit = (e) => {
        ...
        if (editId) {
            sendData.id = editId
            EmailTemplateMutateUpdate.mutate(sendData)
            return
        }

        EmailTemplateMutate.mutate(sendData)
    ...
    const EmailTemplateMutate = useMutation(
        (data) => CampaignServices.EmailTemplate(data),
        {
            onSuccess: (data) => {
                Swal.fire({
                    title: "Succssfull",
                ...
                queryClient.refetchQueries("email-template-list");
    ...
    const EmailTemplateMutateUpdate = useMutation(
        (data) => CampaignServices.EmailTemplateUpdate(data),
        {
            onSuccess: (data) => {
                Swal.fire({
                    title: "Succssfull",
    ...
    const {
        ...
        ["email-template-list", currentPage],
        () => {
            ...
            return CampaignServices.EmailTemplateList(formattedQueryParams)
        },
        {
    ...
    const deleteCampaignTemplate = useMutation(
        (formdata) => {
            return CampaignServices.DeleteEmailTemplate(formdata);
        },
        {
            onSuccess: (data) => {
                Swal.fire({
                ...
                currentPage == 1 ? queryClient.refetchQueries("email-template-list") : queryClient.refetchQueries(["email-template-list", 1]);
                setCurrentPage(1)
            },
            onError: (err) => {
                Swal.fire({
    ...
    return (
        <Row>
            ...
            <Col md="12" className="mb-2">
              ...
              <IsAccessibleMethod
                                                      method={Object.keys(PermissionSets.Campign.Campaign_Template.Create)[0]}
                                                      route={window.location.pathname}
                                
                                                    >
            <Col md="12" className="mb-2">
                <FormGroup className='common-formgroup'>
                    <Button type="submit" onClick={handleSubmit} disabled={EmailTemplateMutate?.isLoading} className='btn btn-style1'>{EmailTemplateMutate?.isLoading ? <ButtonLoader /> :editId?"Update Template": "Create Template"}</Button>
                </FormGroup>
            </Col>
            </IsAccessibleMethod>
            ...
            <Col md="12" className='mb-2'>
                ...
                <TemplateListing handleEdit={handleEditEmail} deleteCampaignTemplate={deleteCampaignTemplate} currentPage={currentPage} TemplateList={EmailTemplateList} isLoad={isEmailTemplateLoad} handlePageChange={handlePageChange} handleDelete={handleDelete} pageSize={pageSize} />
            </Col>
