...
const ShareDataFranchiseView = () => {
  ...
  const { data: franchiseList, isLoading: isLoadedFranchiseList } = useQuery(
    ["share-data-franchise"],
    () => {
        let queryParams = ""
      queryParams += loggedInStaffId ? 'staff=' + loggedInStaffId : ''
      
   return    ShareDataServices.getFranchiseList(queryParams)},
    {
  ...
  const { data, isLoading: isLoadingData } = useQuery(
    [
      "bridengroomdata-listing-share-data-franchise",
      pageNumber,
      franchiseData?.value ? franchiseData?.value : "",
      limit,
    ],
    () =>
      ShareDataServices.getShareDataGroomList(
        `?page=${pageNumber}&page_size=${limit}&is_approve=${1}${franchiseData?.value ? "&tenant=" + franchiseData.value : ""
        }`
      ),
    {
      onSuccess: (data) => {
  ...
  const handleCheckedUser = (e, id) => {
    ...
    if (isChecked) {
      ...
      setCheckBoxList((prev) => [
        ...prev,
        { tenant: franchiseData?.value, brideandgroom: id },
      ]);
    } else {
  ...
  const handleUpdate = () => {
    if (!franchiseData || franchiseData?.value == "") {
      setErr({ ...err, franchise: "Please select Franchise First" });
      return;
    }
    ...
    // if (loggedInStaffId) {
    //   sendData = checkboxList?.map((each) => {
    //     return {
    //       tenant: orgData?.value,
    //       staff: each?.brideandgroom,
    //     };
    //   });
    //   mutationStaffShare.mutate(sendData)
    //   return
    // }
    // console.log(sendData)
    mutation.mutate(sendData);
    // console.log(checkboxList, franchiseData);
  };
  ...
  return (
    <Wrapper>
      <div className="sharedata-view-wrapper">
            ...
            <div className="search-input">
              ...
              {!isLoadedFranchiseList && (
                <Select
                  ...
                  options={[
                    { value: "", label: "Please select Franchise", data: "" },
              ...
              {err?.franchise && (
                <p className="text-danger">{err?.franchise}</p>
              )}
            </div>
        ...
        <Row>
          {franchiseData?.data &&
            Object.keys(franchiseData?.data).length > 0 && (
            ...
            <Row className="mb-5">
              ...
              <Col xs="12" md="6" lg="4" xxl="3" className="mb-3">
                <div className="choose-membership-list">
                  ...
                  <div className="membership-info">
                    <h4>Price </h4>
                    <h3> Ca$ {franchiseData?.data?.cost || 0}</h3>
                  </div>
                </div>
              </Col>
              <Col xs="12" md="6" lg="4" xxl="3" className="mb-3">
                <div className="choose-membership-list">
                  ...
                  <div className="membership-info">
                    ...
                    <h3> Ca$ {franchiseData?.balance || 0}</h3>
                  </div>
                </div>
              </Col>
              <Col xs="12" md="6" lg="4" xxl="3" className="mb-3">
                <div className="choose-membership-list">
                  ...
                  <div className="membership-info">
                    ...
                    <h3>{franchiseData?.data?.no_of_subscription || 0}/0</h3>
                  </div>
                </div>
              </Col>
              ...
              <Col xs="12" md="6" lg="4" xxl="3" className="mb-3">
                <div className="choose-membership-list">
                  ...
                  <div className="membership-info">
                    ...
                    {(franchiseData?.data?.payment_gateway?.length &&
                      franchiseData?.data?.payment_gateway?.map((elem) => {
                        return (
                          <img
                            src={
                              (elem?.logo_img &&
              ...
              <div className="choose-membership-list">
                ...
                <div className="membership-info">
                  ...
                  <h3>
                    {" "}
                    {franchiseData?.data?.domain_setup ? (
                      <IoMdCheckmark />
                    ) : (
                      <IoClose />
                    )}
            ...
            <Table responsive>
              ...
              <tbody>
                {data?.data?.results?.map((each, index) => {
                  ...
                  return (
                    <tr>
                      {each?.share_count == 0 && franchiseData?.value ? (
                        <td>
                          <FormGroup className="mb-0 mb-sm-0">
                            <Input
                              id="select-all"
...
export default ShareDataFranchiseView;
