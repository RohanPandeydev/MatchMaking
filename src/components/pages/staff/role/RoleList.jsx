...
const RoleList = () => {
  ...
  const { data: roleList, isLoading: isRoleLoad } = useQuery(
    ["role-list", pageNumber],
    () => StaffServices.roleList(`?page=${pageNumber}&page_size=${limit}`),
    {
  ...
  const DeleteStaff = useMutation((data) => StaffServices.deleteStaff(data), {
    onSuccess: (data) => {
      ...
      queryClient.refetchQueries(["role-list", pageNumber]);
    },
    onError: (err) => {
  ...
  return (
    <Wrapper>
        ...
        <Row>
          ...
          <Col xs="12" lg="6">
            <ul className="common-head-list justify-content-lg-end">
             ...
             <IsAccessibleMethod
                          method={Object.keys(PermissionSets.staff.Role.Create)[0]}
      ...
      {isRoleLoad ? (
        <Loader />
      ) : roleList?.data?.results?.length == 0 ? (
        <NoActiveDataFound msg="No Staff Found" />
      ) : (
            <Row>
              { roleList?.data?.results?.map((each) => {
                return (
                    <Col md="3" sm="6">
      ...
      {!isRoleLoad && roleList?.data?.results?.length > 0 && (
        <Pagination count={roleList?.data?.count} pageSize={initialLimit} />
      )}
    </Wrapper>
  );
};
