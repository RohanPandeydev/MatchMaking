...
const PermissionList = () => {
    ...
    const { data: teamList, isLoading: isTeamLoad } = useQuery(
        ["permission-list", pageNumber],
        () =>
            StaffServices.permissionList(`?page=${pageNumber}&page_size=${limit}`),
        {
    ...
    const DeleteStaff = useMutation((data) => StaffServices.deleteStaff(data), {
        onSuccess: (data) => {
            ...
            queryClient.refetchQueries(["permission-list", pageNumber]);
        },
        onError: (err) => {
    ...
    const handleNav = (id) => {
        if (id) {
            nav("/permissionassign/form/" + btoa(id));
            return;
        }
        nav("/permissionassign/form");
    };
    ...
    return (
        <Wrapper>
            ...
            {isTeamLoad ? (
                ...
                <NoActiveDataFound msg="No Permission Found" />
            ) : (
                        ...
                        <div className="permission-list">
                            <div className="staff-head">
                                <div className="staff-head-dtls">
                                    ...
                                    <div className="staff-head-info-wrap right">
                                        <div>
                                            {each?.content?.map((ele) => {
                                                return (
                                                    <p
                                                        className="department-permission-listing"
                                                        key={ele?.parent}
                                                    >
                                                        <strong>{ele?.parent}</strong>
                                                        <br />
                                                                   ...
                                                                   {child?.child} -{" "}
                                                                    {child?.methods?.map((method) => (
                                                                        <div className="permission-box">
                                                                            <p key={method?.name} className="child-permission">
                                                                                {method?.name}
                                                                            </p>
                                                                            <ul>
                                                                                {method?.name}
                                                                                {method.method.map((each) => (
                                                                                    <li>{each}</li>
                                                                                ))}
                                                                            </ul>
                                                                        </div>
                                            ...
                                            <div>
                                                {each?.staffs?.map((staff) => {
                                                    const staffName =
                                                        ...
                                                        "";
                                                    const staffRole =
                                                        `${staff.role?.name} ${staff.role?.code}` || "";
                                                    return (
                                                        <div>
                                                            {staffName}({staffRole})
                                                        </div>
