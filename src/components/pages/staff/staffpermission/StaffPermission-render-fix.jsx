...
const StaffPermission = ({ selectedPermissions, setSelectedPermissions }) => {
  ...
  const handleNestedPermissionToggle = (parentIndex, methodIndex, nestedKey, isChecked, isDirectMethod = false) => {
    const updatedData = _.cloneDeep(permissionData);
    const parent = updatedData[parentIndex];
    const method = isDirectMethod
      ? parent.methods[methodIndex]
      : parent.children.flatMap(child => child.methods)[methodIndex];

    method.value[nestedKey] = isChecked;
  ...
  const handleChildCheckbox = (parentIndex, childIndex, isChecked) => {
    const updatedData = _.cloneDeep(permissionData);
    const parent = updatedData[parentIndex];
    const child = parent.children[childIndex];

    // Check the child and all its methods
    child.isChecked = isChecked;
    child.methods.forEach(method => {
      method.isChecked = isChecked;
      if (typeof method.value === "object") {
