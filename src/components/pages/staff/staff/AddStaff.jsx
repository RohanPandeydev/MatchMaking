...
const AddStaff = () => {
  ...
  const { data: permissionList, isLoading: isPermissionLoad } = useQuery(
    ["permission-list-nopagination"],
    () => StaffServices.permissionListWithoutPagination(),
    {
      refetchOnWindowFocus: false,
      select: (data) => {
        // console.log("Data Franchise ", data?.data);
  ...
  const { data: staffDetails, isLoading: isStaffLoad } = useQuery(
    ...
    () => StaffServices.staffDetails({ id: id }),
    {
      ...
      onSuccess: (data) => {
        ...
        formik.setFieldValue("role", response?.role?.id);
        formik.setFieldValue("permission", response?.permission?.id);
