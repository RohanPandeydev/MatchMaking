...
import noofChildren from "../../../../utils/JSON/NoOfChildren";
import { MasterNoofChildren } from "../../../../helper/ValidationHelper/Validation";
import { update } from "lodash";

const AddNoOfChildren = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: MasterNoofChildren,
  ...
  const AddNoOfChildren = useMutation(
    (formdata) => MasterServices.Add(formdata),
    {
      onSuccess: (data) => {
        ...
        formik.resetForm();
        nav("/master/noofchildren/");

        queryClient.refetchQueries("all-noofchildren-list");
        return;
      },
      onError: (err) => {
          ...
          <Col md="6">
            <FormGroup className="common-formgroup">
              ...
              <Input
                ...
                type="text"
              >
                {/* <option value={""}>Please Select No of Children</option> */}
                {/* {noofChildren?.map((each, index) => {
                    return (
          ...
          <Col md="12">
            <FormGroup className="common-formgroup">
              <Button
                ...
                type="submit"
                disabled={AddNoOfChildren?.isLoading || update?.isLoading}
              >
                {AddNoOfChildren?.isLoading || update?.isLoading ? (
                  <ButtonLoader />
                ) : (
                  "Submit"
                )}
...
export default AddNoOfChildren;
