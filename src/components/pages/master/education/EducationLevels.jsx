...
const EducationLevels = () => {
  ...
  const deleteMutation = useDeleteMasterContent("all-education-list");
  const updateMutation = useUpdateContent("all-education-list");
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => row.description && JSON.parse(row?.description)?.level || "N/A",
          ...
          <div className="master-action-btn-wrap">
            ...
            >
              <Button
                ...
                onClick={() =>
                  nav("/master/educationlevels/add/" + btoa(row?.id))
                }
              >
                {" "}
                <FaEdit />{" "}
  ...
  const handleNav = () => {
    nav("/master/educationlevels/add");
  };
  ...
  const { data, isLoading } = useFetchMasterSectionData(url, "all-education-list", search)
