...
const ManglikStatuses = () => {
  const url = getValueBetweenMasterAndAdd(window.location.href);
  const nav = useNavigate();
  const deleteMutation = useDeleteMasterContent("all-manglik-list");
  const updateMutation = useUpdateContent("all-manglik-list");
          ...
          <div className="master-action-btn-wrap">
            ...
            >
              <Button
                ...
                onClick={() =>
                  nav("/master/manglikstatuses/add/" + btoa(row?.id))
                }
              >
                {" "}
                <FaEdit />{" "}
  ...
  const handleNav = () => {
    nav("/master/manglikstatuses/add");
  };
  ...
  const { data, isLoading } = useFetchMasterSectionData(url, "all-manglik-list", search)
...
export default ManglikStatuses;
