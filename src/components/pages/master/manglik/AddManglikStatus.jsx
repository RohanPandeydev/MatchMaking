...
import manglikStatuses from "../../../../utils/JSON/ManglikStatus";
import { MasterManglikStatus } from "../../../../helper/ValidationHelper/Validation";

const AddManglikStatus = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: MasterManglikStatus,
  ...
  const AddManglik = useMutation((formdata) => MasterServices.Add(formdata), {
    onSuccess: (data) => {
      ...
      formik.resetForm();
      nav("/master/manglikstatuses");

      queryClient.refetchQueries("all-manglik-list");
      return;
    },
    onError: (err) => {
          ...
          <Col md="6">
            <FormGroup className="common-formgroup">
              ...
              <Input
                ...
                type="text"
              >
                {/* <option value={""}>Please Select Manglik Status</option>
                {manglikStatuses?.map((each, index) => {
                    return (
          ...
          <Col md="12">
            <FormGroup className="common-formgroup">
              <Button
                ...
                type="submit"
                disabled={AddManglik?.isLoading || Update?.isLoading}
              >
                {AddManglik?.isLoading || Update?.isLoading ? (
                  <ButtonLoader />
                ) : (
                  "Submit"
                )}
...
export default AddManglikStatus;
