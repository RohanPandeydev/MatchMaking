...
const Body = () => {
  ...
  const deleteMutation = useDeleteMasterContent("all-body-type-list");
  const updateMutation = useUpdateContent("all-body-type-list");

  const bodyTypeOptions = [
    { id: 1, bodyType: "Slim" },
    { id: 2, bodyType: "Athletic" },
    { id: 3, bodyType: "Average" },
    { id: 4, bodyType: "Heavy" },
    { id: 5, bodyType: "Muscular" },
    { id: 6, bodyType: "Curvy" },
    { id: 7, bodyType: "Stocky" },
  ];
  const columns = [
    {
      name: "Sl No.",
          ...
          <div className="master-action-btn-wrap">
            ...
            >
              <Button
                ...
                onClick={() => nav("/master/body/add/" + btoa(row?.id))}
              >
                {" "}
                <FaEdit />{" "}
              </Button>
  ...
  const { data, isLoading } = useFetchMasterSectionData(url, "all-body-type-list", search)
