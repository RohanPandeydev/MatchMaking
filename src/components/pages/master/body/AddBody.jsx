...
import bodyTypes from "../../../../utils/JSON/BodyType";
import { MasterBodyType } from "../../../../helper/ValidationHelper/Validation";
...
const AddBody = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: MasterBodyType,
  ...
  const Update = useMutation((formdata) => MasterServices.Update(formdata), {
    onSuccess: (data) => {
      ...
      queryClient.refetchQueries("all-body-type-list");
      return;
    },
    onError: (err) => {
  ...
  const AddBodyType = useMutation((formdata) => MasterServices.Add(formdata), {
    onSuccess: (data) => {
      console.log("Data==>", data?.data);
      Swal.fire({
        title: "Successfull",
          ...
          <Col md="12">
            <FormGroup className="common-formgroup">
              <Button
                ...
                type="submit"
                disabled={AddBodyType?.isLoading||Update?.isLoading}
              >
                {AddBodyType?.isLoading ||Update?.isLoading? <ButtonLoader /> : "Submit"}
              </Button>
            </FormGroup>
          </Col>
        </Row>
