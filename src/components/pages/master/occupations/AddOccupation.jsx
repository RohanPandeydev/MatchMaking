...
import occupations from "../../../../utils/JSON/occupation";
import { MasterOccupation } from "../../../../helper/ValidationHelper/Validation";
...
const AddOccupations = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: MasterOccupation,
  ...
  const AddOccupation = useMutation(
    (formdata) => MasterServices.Add(formdata),
    {
      onSuccess: (data) => {
        ...
        formik.resetForm();
        nav("/master/occupations");

        queryClient.refetchQueries("all-occupation-list");
        return;
      },
      onError: (err) => {
  ...
  const { data: PageId, isLoading: isPage } = useQuery(
    ["all-pageId-list-occupation", url],
          ...
          <Col md="12">
            <FormGroup className="common-formgroup">
              <Button
                ...
                type="submit"
                disabled={AddOccupation?.isLoading}
              >
                {AddOccupation?.isLoading ? <ButtonLoader /> : "Submit"}
              </Button>
            </FormGroup>
          </Col>
        </Row>
