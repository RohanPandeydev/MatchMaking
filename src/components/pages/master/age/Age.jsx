...
const Age = () => {
  ...
  const deleteMutation = useDeleteMasterContent("all-age-list");
  const updateMutation = useUpdateContent("all-age-list");
          ...
          <div className="master-action-btn-wrap">
            ...
            >
              <Button
                ...
                onClick={() => nav("/master/age/add/" + btoa(row?.id))}
              >
                {" "}
                <FaEdit />{" "}
              </Button>
  ...
  const { data, isLoading } = useFetchMasterSectionData(url, "all-age-list", search)
