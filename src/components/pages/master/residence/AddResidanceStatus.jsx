...
import ResidenceStatuses from "./ResidenceStatuses";
import { MasterResidanceStatus } from "../../../../helper/ValidationHelper/Validation";
import residenceStatuses from "../../../../utils/JSON/ResidanceStatus";

const AddResidanceStatus = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: MasterResidanceStatus,
  ...
  const AddResidanceStatus = useMutation(
    (formdata) => MasterServices.Add(formdata),
    {
      onSuccess: (data) => {
        ...
        formik.resetForm();
        nav("/master/residencestatus/");

        queryClient.refetchQueries("all-lang-list-residance-status");
        return;
      },
      onError: (err) => {
  ...
  const { data: PageId, isLoading: isPage } = useQuery(
    ["all-pageId-list-residance-status", url],
          ...
          <Col md="6">
            <FormGroup className="common-formgroup">
              ...
              <Input
                ...
                type="text"
              >
                {/* <option value={""}>Please Select Residance Status</option>
                {residenceStatuses?.map((each, index) => {
                  return (
          ...
          <Col md="12">
            <FormGroup className="common-formgroup">
              <Button
                ...
                type="submit"
                disabled={AddResidanceStatus?.isLoading || Update?.isLoading}
              >
                {AddResidanceStatus?.isLoading || Update?.isLoading ? (
                  <ButtonLoader />
                ) : (
                  "Submit"
                )}
...
export default AddResidanceStatus;
