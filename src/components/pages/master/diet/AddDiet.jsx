...
import { MasterDiet } from "../../../../helper/ValidationHelper/Validation";
...
const AddDiet = () => {
  ...
  const submitHandler = (data) => {
    ...
    AddDiet.mutate({ name: data?.name,description:JSON.stringify(data?.name?.replace(/\s+/g, "_")), category: ids });
  };

  const AddDiet = useMutation((formdata) => MasterServices.Add(formdata), {
    onSuccess: (data) => {
      ...
      queryClient.refetchQueries("all-diet-list");
      return;
    },
    onError: (err) => {
          ...
          <Col md="12">
            <FormGroup className="common-formgroup">
              <Button
                ...
                type="submit"
                disabled={AddDiet?.isLoading}
              >
                {AddDiet?.isLoading ? <ButtonLoader /> : "Submit"}
              </Button>
            </FormGroup>
          </Col>
        </Row>
