...
const Diet = () => {
  ...
  const deleteMutation = useDeleteMasterContent("all-diet-list");
  const updateMutation = useUpdateContent("all-diet-list");
          ...
          <div className="master-action-btn-wrap">
            ...
            >
              <Button
                ...
                onClick={() => nav("/master/diet/add/" + btoa(row?.id))}
              >
                {" "}
                <FaEdit />{" "}
              </Button>
  ...
  const { data, isLoading } = useFetchMasterSectionData(url, "all-diet-list", search)
