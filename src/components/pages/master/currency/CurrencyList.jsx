...
const CurrencyList = () => {
  ...
  const deleteMutation = useDeleteMasterContent("all-complexion-list");
  const updateMutation = useUpdateContent("all-complexion-list");
  ...
  const columns = [
    ...
    {
      name: "Currency",
      selector: (row) => row?.description && JSON.parse(row?.description)?.symbol && JSON.parse(row?.description)?.symbol || "N/A",
      ...
      selector: (row) => (
        <>
          ...
          <div className="master-action-btn-wrap">
            <IsAccessibleMethod
              methodName={PermissionSets?.master?.CurrencyList?.Update}
              pathname={window.location.pathname}
            >
              <Button
                className="btn green-btn"
                onClick={() => nav("/master/currency/add/" + btoa(row?.id))}
              >
                {" "}
                <FaEdit />{" "}
              </Button>
            ...
            <IsAccessibleMethod
              methodName={PermissionSets?.master?.CurrencyList?.Delete}
              pathname={window.location.pathname}
            >
              <Button
            ...
            <IsAccessibleMethod
              methodName={PermissionSets?.master?.CurrencyList?.Status}
              pathname={window.location.pathname}
            >
              <Button
  ...
  const { data, isLoading } = useFetchMasterSectionData(url, "all-currency-list", search)
      ...
      <div className="common-db-head mb-4">
        <div className="align-items-center row">
          <div className="d-inline-flex col-md-7 col-lg-6">
            ...
            methodName={PermissionSets?.master?.CurrencyList?.Create}
            pathname={window.location.pathname}
          >
