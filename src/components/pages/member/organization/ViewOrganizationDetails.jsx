...
import { NavLink, Button, Col, Modal, ModalBody, Nav, NavItem, Row, TabContent, TabPane, Label, FormGroup, Input } from "reactstrap";
...
import { IoCallOutline, IoCheckmarkCircleOutline, IoClose } from "react-icons/io5";
...
const ViewDetails = () => {
  ...
  const { data: organization_details, isLoadOrganizationDetails } = useQuery(
  ...
  const { data, isLoading: isLoadingData } = useQuery(
    ["bridengroomdata-listing-list-by-id-org", pageNumber, id],
    () =>
  ...
  useEffect(() => {
    try {
      const decodeId = organizationId && atob(organizationId);

      organizationId && setId(() => decodeId || "");
    } catch (error) {
    ...
    },
    {
      onSuccess: (data) => {
        ...
        queryClient.refetchQueries([
          "bridengroomdata-listing-list-by-id-org",
          pageNumber,
        ]);
      },
      onError: (err) => {
              ...
              <div className="org-profile-details-wrap mb-4">
                <div className="member-org-profile-name-wrap">
                  <div className="profile-dtls-prof-img offline">
                    <div className="prof-img">
                      <img
                        src={
                          (organization_details?.image_url &&
                            `${config.apiUrl}${organization_details?.image_url}`) ||
                          profileUser
                        }
                        alt=""
                      />
                  ...
                  <div className="profile-dtls-prof-info">
                    ...
                    <h3>Owner : - {organization_details?.owner_name || ""}</h3>
                    <h5>
                      {organization_details?.is_verified
                        ? "Verified"
                        : "Unverified"}
                    </h5>
                  </div>
                  <div className="member-code">{organization_details?.code || "N/A"}</div>
                </div>
                ...
                <div className="member-org-profile-info-wrap">
                  ...
                  >
                    <p>
                      <span>Email Id :</span>{" "}
                      {organization_details?.user?.email || ""}
                    </p>
                    <p>
                      <span>Phone Number :</span>{" "}
                      {organization_details?.phone || ""}
                    </p>
                    <p>
                      <span>Occupation :</span> Teacher
                    </p>
                    <p>
                      <span>Country :</span> {organization_details?.country || ""}
                    </p>
                    <p>
                      <span>State :</span> {organization_details?.state || ""}
                    </p>
                    <p>
                      <span>City :</span> {organization_details?.city || ""}
                    </p>
                    <p>
                      <span>Address :</span> {organization_details?.address || ""}
                    </p>
                  </IsAccessibleMethodBMS>
                  <p>
                    <span>Apps :</span>
                    {organization_details?.apps && JSON.parse(organization_details?.apps)?.android && <FcAndroidOs /> || ""}
                    {organization_details?.apps && JSON.parse(organization_details?.apps)?.ios && <FaAppStoreIos /> || ""}
                    {organization_details?.apps && JSON.parse(organization_details?.apps)?.api && <SiPostman /> || ""}
                  </p>
                  <IsAccessibleMethodBMS
                  ...
                  >
                    <>
                      <p>
                        ...
                        {organization_details?.subscription?.name || "N/A"} (
                        {organization_details?.subscription?.cost || "0"} ca$)
                      </p>
                      <p>
                        <span>No Of Subscriptions Purchased :</span>{" "}
                        {organization_details?.subscription_count || 0}
                      </p>
                      <p>
                        <span>Balance:</span> {organization_details?.balance || 0}
                      </p>
                    </>
                  </IsAccessibleMethodBMS>
                  ...
                  <p>
                    ...
                    {organization_details?.brideandgroom_count || 0}
                  </p>
                  <p>
                    <span>Registration number:</span>{" "}
                    {organization_details?.registration_num || 0}
                  </p>
                  <p>
                    <span>Verification number:</span>{" "}
                    {organization_details?.verification_num || 0}
                  </p>
                </div>
              </div>
